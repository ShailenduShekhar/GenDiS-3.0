...
function buildTable(data) {
	...
	if (data.length === 0) {
		...
		for (let i = 0; i < data.length; i++) {
			row = `<tr>
        <td>
          <a class="sf-id" href="/class_display/${data[i]._id}" target="_blank">
            ${data[i]._id}
          </a>
          <img src="static/img/popup-link-icon.svg" alt="link to another page">
        </td>
        <td>${data[i].des}</td>
        <td>${data[i].num_fold}</td>
        <td>${data[i].num_sf}</td>
        <td>${data[i].num_seq}</td>
        <td>${data[i].num_tp}</td>
      </tr>`;
			tbody.append(row);
		}
...
$.ajax({
	url: '/class_data', /* Flask route here and not the name of that function */
